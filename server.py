...
def scout_player():
    if request.method == 'POST':
        ...
        division = str(request.form['division'])

        return redirect(url_for('player_results', division=division, code=code))
    
    return render_template('index.html')
    
@app.route('/results/<division>/<code>', methods=['GET'])
def player_results(division, code):
        ...
        player_comparisons = player_comps[code][0] + " • " + player_comps[code][1] + " • " + player_comps[code][2]
        ...
        if ncaa_table.empty:
            ...
            ovr_data = ((94.9 - ((94.9 - 70.5) * means - 1) / ((maximum + 150) - 1)) + .1).apply(lambda x: round(x, 1))
        ...
        ncaa_table_html = tabulate(ncaa_table, headers="keys", tablefmt="html", showindex=False)

        return render_template('table.html', table_results=ncaa_table_html, arch_title=arch_title,player_comparisons=player_comparisons, division=division, code=code)
